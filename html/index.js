...
window.addEventListener('message', function(event) {
        ...
        yordinvector4Coords(event.data);
    })
})
...
function yordivector4Copy(){
    const copyText = document.getElementById("yordivector4Coords").textContent;
    const textArea = document.createElement('textarea');
    textArea.textContent = copyText;
    document.body.append(textArea);
    textArea.select();
...
function yordinvector4Coords(data) {
    if (data.type === 'vector4') {
        yordicoordsvector4Start(data);
    }
}
...
function yordicoordsvector4Start(data){
    document.querySelector("#yordivector4Coords").textContent = data.text;
}